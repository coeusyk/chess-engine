...
/**
 * Computes the Polyglot Zobrist hash for a given Board position.
 *
 * The 781-entry random array is the standard Polyglot table (identical across
 * all conforming implementations).  Layout:
 *   [0..767]   piece-square keys — index = piece_index*64 + polyglot_sq
 *              piece_index = (piece_type-1)*2 + pivot
 *              pivot = 0 for black, 1 for white
 ...
 *   [772..779] en-passant file a..h (only when a pawn can actually capture)
 *   [780]      white to move
 *
 * Square conversion:
 *   Engine uses a8=0, h1=63 (rank_idx=sq/8 where 0=rank8).
 *   Polyglot uses a1=0, h8=63 (rank=sq/8 where 0=rank1).
 *   polyglotSq = (7 - engineSq/8)*8 + (engineSq%8)
 */
public final class PolyglotKey {

    private static final long[] RANDOM = {
    ...
    /**
     * Computes the Polyglot Zobrist hash for the given board position.
     */
    public static long compute(Board board) {
        long key = 0L;

        // --- Pieces ---
        // piece_index = (pieceType - 1) * 2 + pivot  (pivot: 0=black, 1=white)
        for (int pt = 1; pt <= 6; pt++) {
            long wBB = getBitboard(board, pt, true);
            long bBB = getBitboard(board, pt, false);
            int baseW = ((pt - 1) * 2 + 1) * 64;
            int baseB = ((pt - 1) * 2    ) * 64;
            long wBits = wBB;
            while (wBits != 0) {
                int sq = Long.numberOfTrailingZeros(wBits);
                key ^= RANDOM[baseW + toPolyglotSq(sq)];
                wBits &= wBits - 1;
            }
            long bBits = bBB;
            while (bBits != 0) {
                int sq = Long.numberOfTrailingZeros(bBits);
                key ^= RANDOM[baseB + toPolyglotSq(sq)];
                bBits &= bBits - 1;
            }
        }
        ...
        if (ca[0]) key ^= RANDOM[768]; // white kingside
        if (ca[1]) key ^= RANDOM[769]; // white queenside
        if (ca[2]) key ^= RANDOM[770]; // black kingside
        if (ca[3]) key ^= RANDOM[771]; // black queenside

        // --- En passant ---
        // Only hash EP file when a pawn of the side to move can actually capture.
        int ep = board.getEpTargetSquare();
        if (ep >= 0) {
            int captureColor = board.getActiveColor();
            long capturingPawns = Piece.isWhite(captureColor)
                ? board.getWhitePawns()
                : board.getBlackPawns();
            int epFile = ep % 8;
            // Capturing pawns sit one rank "behind" the ep target (in standard-chess view).
            // Engine sq increases toward rank 1: white pawns are at ep+8, black at ep-8.
            int pawnBase = ep + (Piece.isWhite(captureColor) ? 8 : -8);
            boolean canCapture = (epFile > 0 && (capturingPawns & (1L << (pawnBase - 1))) != 0)
                              || (epFile < 7 && (capturingPawns & (1L << (pawnBase + 1))) != 0);
            if (canCapture) {
        ...
        // --- Turn ---
        // Polyglot convention: XOR with RANDOM[780] when it is WHITE to move.
    ...
    /** Engine sq → Polyglot sq.  Engine: a8=0, h1=63.  Polyglot: a1=0, h8=63. */
    private static int toPolyglotSq(int engineSq) {
        return (7 - engineSq / 8) * 8 + (engineSq % 8);
    }

    private static long getBitboard(Board board, int pieceType, boolean white) {
        return switch (pieceType) {
            case 1 -> white ? board.getWhitePawns()   : board.getBlackPawns();
            case 2 -> white ? board.getWhiteKnights() : board.getBlackKnights();
            case 3 -> white ? board.getWhiteBishops() : board.getBlackBishops();
            case 4 -> white ? board.getWhiteRooks()   : board.getBlackRooks();
            case 5 -> white ? board.getWhiteQueens()  : board.getBlackQueens();
            case 6 -> white ? board.getWhiteKing()    : board.getBlackKing();
            default -> 0L;
        };
    }
}
