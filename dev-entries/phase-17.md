...
- **The five stale regression expectations re-probed under the repaired candidate**, per this step's instruction to repeat probes rather than reuse the pre-repair diagnostic's conclusions, since the repair is itself a search-shape change:
...
- Broke (expected, not a regression): the same five pre-existing goldens flagged by the original mechanism-gate commit drift again, for the same underlying reason (a real, intentional search-shape change), now re-verified and re-updated against the repaired candidate specifically rather than carried over from the pre-repair diagnostic.

**Measurements:** See the Gate 1 rerun table, per-position delta table, and `pvNodeEvals` before/after figures above.

**Status:** Phase 17 Step 3 (formal correctness gate) is **BLOCKED on the native Gate 2 rerun**, not complete and not rejected. Everything resolvable in this WSL session is done and green: the PV-node-propagation question is resolved (was a defect, is now repaired), Gate 1 is revalidated and passes on the repaired candidate, the full relevant test suite is green, all five stale regression goldens are re-verified and updated with evidence, and PV legality holds. What remains is exactly one native-Windows action: rerun `tools/p17-2-native-throughput.ps1` (or the equivalent canonical protocol) against this branch's current HEAD, to produce a throughput number that actually characterizes the repaired candidate, before Step 3 can be marked complete and before Step 4 (SPRT, also separately blocked on #231) can begin. Do not run SPRT before that rerun.
