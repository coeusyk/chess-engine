...
### [2026-09-20] Phase 17 Step 4 executed: H0 accepted, run audited VALID, post-run evidence-plumbing bug fixed (Issues #229, #230)

**Built:**

- **Preserved the actual run's evidence before touching anything.** The native run (host RENEGADE, AMD Ryzen 7 7700X) produced `tools/results/sprt_phase17-pvs_20260920_160513.log` and `.pgn` at the repository root's `tools/results/` (not under the `tools/results/p17-4/20260920-103500/` directory the wrapper itself expected, see below), plus the wrapper's own pre-match evidence in that `20260920-103500/` directory (`00-process-evidence.json`, `01-git-state.txt`, `01b-orchestration-untracked.txt`, build/worktree logs, `05-environment.json`, and both engine JARs). Copied the actual log and PGN into that run's own evidence directory as `06a-sprt-authoritative.log` / `06b-sprt-authoritative.pgn`, recording SHA-256 of the originals before copying and of both the originals and the copies after, confirming byte-for-byte identity in all four hashes; the originals were left untouched.
- **Confirmed candidate/baseline identity from the run's own evidence.** `05-environment.json`'s `candidate_actual_commit`/`baseline_actual_commit` equal their respective frozen commits (`f9b152c`, `ebe513e`), and `candidate_jar_sha256`/`baseline_jar_sha256` match the actual JAR files in the run directory byte-for-byte (recomputed independently, not read back from the JSON that recorded them). This is exactly the guarantee the exact-commit worktree build exists to provide, and it held.
- **Traced the exact cause of the wrapper's post-processing failure.** `tools/p17-4-sprt.ps1` invoked `tools/sprt.ps1` with `2>&1 | Tee-Object -FilePath $sprtLog` and then tried `Get-Content -Path $sprtLog -Raw` to parse the SPRT verdict. Reading `tools/sprt.ps1`'s own source: it accepts no output-path parameter at all; it always computes its own `$LogOut`/`$PgnOut` from `-Tag` and a `Get-Date` timestamp captured when it starts (not the wrapper's own `$timestamp`), and writes both under `tools/results/` directly via its own `StreamWriter` and cutechess's `-pgnout`, never under the wrapper's `$outDir`. Separately, and this is the actual defect: `tools/sprt.ps1` reports nearly all of its progress via `Write-Host` (`Write-Host $line` for every line of cutechess's own output, plus its own status messages), which writes to PowerShell's information stream, not the success or error streams; `2>&1` merges only the error stream into the success stream, so none of that `Write-Host` output was ever captured by the wrapper's `Tee-Object`. The match itself was unaffected: `tools/sprt.ps1`'s own authoritative log and PGN contain the complete, correct transcript ending in `Finished match`, the per-player termination breakdown, and the `SPRT: llr -3.1 ... H0 was accepted` line, all written independently of the broken wrapper-side capture. The post-processing failure occurred strictly after cutechess had already finished the match and emitted its decision; it never touched, delayed, or altered any game.
- **Audited the five "No result" games (cutechess's games 15-19) using the actual log and PGN.** Their PGN `[Termination]` tag is `"unterminated"` (cutechess's tag for a game cut off in progress), not any crash/timeout/error-specific tag; their ply counts are short and varied (31, 26, 16, 9, 5), consistent with five mid-game snapshots at whatever point each one happened to be when the match stopped, not a uniform failure signature. Chronologically, the log's SPRT-boundary-crossing line and all five `{No result}` lines appear in the same block, immediately after the 14th scored game (game 12) finished, with no further `Started game` lines afterward and `Finished match` as the log's last line: this is cutechess's ordinary behavior under `-concurrency 6` with an active `-sprt` flag, where games still in flight when the boundary is crossed are cancelled rather than played out, since the SPRT decision no longer depends on them. Classified as **normal post-SPRT cancellation, not engine failures.**
- **Audited match configuration for NEW/OLD symmetry directly from `tools/sprt.ps1`'s argument construction**, not assumed from the wrapper having printed the intended values: both engines are launched with identical `cmd`/`arg=--add-modules jdk.incubator.vector`/`proto=uci`/`option.Threads=1`, and identical `Hash=16` via `-NewOptions`/`-OldOptions`; `-each tc=5+0.05` applies one time control to both engines (no per-engine override exists in this script); `-repeat` pairs each opening twice with colors swapped, confirmed directly in the PGN (games 1/2, 3/4, ... 17/18 each share an identical FEN with White/Black reversed). NEW played White in every odd-numbered game and Black in every even-numbered game, matching `-repeat`'s alternation exactly, and the reported per-color split (White 0-6-1, Black 0-7-0) is internally consistent with that. Candidate and baseline were not reversed: `-New $candidateJarPath`/`-Old $baselineJarPath` map directly to the paths `Build-FrozenEngineJar` returned for the candidate and baseline commits, and the PGN's `[White "NEW"]`/`[White "OLD"]` tags track the same `-engine name=NEW`/`name=OLD` labels for those same two JARs.
- **Inspected all 14 completed games and the PGN/log for failure signatures.** Grepped both files for `illegal`, `disconnect`, `timeout`, `crash`, `stall`, `error`, `communication`, and `forfeit` (case-insensitive): zero matches in either. All 13 decisive games and the one draw carry `[Termination "adjudication"]` or ended by 3-fold repetition; none carry a crash/timeout/illegal-move tag. Where both engines' move-comment evaluations could be compared near a game's end, NEW's and OLD's self-reported scores agree in sign and rough magnitude from each side's own perspective (for example, game 1's final comments run roughly +7.85/+7.60/+8.09 for one side against -7.59/-7.84/-8.34 for the other), the pattern expected from a real, mutually-recognized decisive advantage rather than a broken or inverted candidate evaluation.
- **Fixed the post-processing bug in `tools/p17-4-sprt.ps1`.** Changed `2>&1` to `*>&1` on the `tools/sprt.ps1` invocation so `Write-Host` output actually flows into the wrapper's own transcript. Beyond that minimal fix, added logic to locate `tools/sprt.ps1`'s own authoritative log/pgn after the call returns (newest files matching this run's `-Tag` under `tools/results/`) and copy them into the run's evidence directory as `06a-sprt-authoritative.log`/`06b-sprt-authoritative.pgn`, and changed the verdict-parsing step to read from that authoritative log when found, falling back to the console transcript only if it wasn't. This removes the wrapper's dependence on a fragile console-transcript capture for something as important as the SPRT verdict string. No PowerShell interpreter is available in this WSL session to run a Pester test against the fix; reviewed by manual read and a brace/paren balance check (42/42, 113/113), consistent with every prior pass on this script. No chess game was run to test the fix, per the task's explicit instruction.
- **Updated `docs/architecture/research/phase17-p17-4-strength-preregistration.md`**: section 8 (Status) now reports the run as complete rather than "preparation only"; a new section 9 (Result) states the score, SPRT decision, and the precise H0 interpretation per section 6's existing verdict semantics, and explicitly reaffirms Gates 1-3's PASS results are unaffected; a new section 10 (Run-validity audit) records the full audit above; a new section 11 records the post-run evidence-plumbing bug separately from match validity, per the task's explicit instruction to keep those two findings distinct.

**Decisions Made:**

- **Validity decision: VALID.** The frozen protocol executed with the correct candidate and baseline JARs (identity independently verified), symmetric engine settings, correctly paired openings, no engine or protocol failure in any of the 14 scored games, and the five uncompleted games are fully explained by ordinary post-SPRT-decision cancellation under concurrency. The surprising 0-13-1 score and H0 verdict are not, by themselves, grounds for invalidation; nothing found in this audit constitutes a concrete execution or configuration failure.
- **H0 accepted means the evidence favors elo0=0 over elo1=+50 under this SPRT; it does not mean a -572 Elo regression is proven, or that PVS is proven weaker in an absolute sense, or that the candidate's true Elo is zero.** The cutechess-reported point estimate (-572.5 +/- nan from 14 games) is explicitly not treated as a reliable effect-size estimate: `+/- nan` means no confidence interval could even be computed, and 14 games is far too small a sample for any point estimate to be meaningful regardless of the interval.
- **Gates 1-3 stand exactly as previously measured and are not revised by this result**: mechanism (Gate 1, PASS, -44.07% main nodes), throughput (Gate 2, PASS, -44.63% median fixed-depth elapsed), correctness (Gate 3, PASS). An efficiency improvement does not imply a playing-strength improvement; Gate 4 is what tests playing strength directly, and this is that test's result for this candidate under this protocol.
- **No follow-up experiment was started.** Per the task's explicit instruction, this pass does not rerun the SPRT, widen or change the bounds, change TC, lower concurrency, tune PVS, test another PVS implementation, or selectively discard this result. The evidentiary record is closed first; any diagnostic follow-up on why the candidate performed this poorly is a separate, later decision, not part of this pass.
- **No PVS/search behavior was changed and no additional game was played.** This pass is entirely evidence preservation, audit, documentation, and a plumbing fix to `tools/p17-4-sprt.ps1`'s post-processing step.

**Broke / Fixed:** Fixed: `tools/p17-4-sprt.ps1`'s post-processing step failed (`Get-Content` on a file `tools/sprt.ps1` never wrote there) because of a `2>&1`-vs-`Write-Host` stream mismatch; the match itself was already complete and correctly recorded by `tools/sprt.ps1`'s own log/pgn when this failure occurred. No search/PVS source touched.
