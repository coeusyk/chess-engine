...
This was the preregistration for Phase 20. Stage 0 and the full Stage 1 requalification passed on 2026-09-24 after the separately preregistered lifecycle repair. Stage 2 ran on native Windows on 2026-09-24; separate-process N=2 median per-worker retention fell below the observed 1T range, so the result remains historically environment-limited and stopped under the original section 13 rule. The first Stage 3 attempt under the post-Stage-2 interpretation amendment (`edd406a9243fcb53c341599d8c04e3fa7f34cab7`) remains stopped and uninterpretable; its exact event ordering cannot be recovered. A separate corrected native fixed-depth Stage 3 run completed on 2026-09-26 and passed its schedule, deterministic-work, and lifecycle gates. Its results and classification are recorded in `dev-entries/phase-20.md`; raw artifacts are in `tools/results/phase20-stage3/20260926-071358/`. Stage 4 and 5 mandatory triggers did not fire; optional Stage 4 characterization and Stage 6 were not run. `Searcher` remains unchanged; no production search, UCI lifecycle, or TT code changed for the harness repair. The Phase 20 base is the post-#245 `develop` head `85b201a2a38f23df09ef2aa758ead77fd6d859ee`.

**Stage 3 attempt (2026-09-24): stopped at the helper-drain gate.** On the native Windows run at commit `55ca1171d3ecaf2891d4ee6e38229557c0640438` (JAR SHA-256 `B7743D6C0EA9EA30F4734933D8091959710D7915F5B27CA431AF41A2301D84A5`), the UCI path returned a main-thread `bestmove`, but the driver did not observe the expected `N-1` helper `event=exit` records and hit its 10-minute watchdog in `Phase20Stage3Harness.search`. This watchdog is an execution guard, not a preregistered acceptance threshold. Its arm, position and warm-up/pass identity were not retained. `workers.csv`, `helpers.csv` and `events.csv` contain headers only, so there are no complete samples to analyze. The run cannot establish whether helpers remained active or their exit diagnostics were lost; helper drainage was not verified. The run was stopped without a retry or production change. No Stage 3 throughput, time-to-depth, H3 or production-overhead classification is made.

**Subsequent harness diagnosis (2026-09-26; no rerun):** The historical attempt above and its stopped-run record remain unchanged. Source ordering confirms the harness could omit helper exits already captured before UCI `bestmove`: helper exit diagnostics are emitted in helper `finally` blocks, while `runSearch` emits/flushed `bestmove` before joining helper futures and emitting `drained`. A deterministic synthetic transcript reproduces the harness accounting failure. This confirms a harness event-accounting defect, but the missing original transcript prevents recovering whether those exact exits preceded bestmove in the 2026-09-24 sample. No production code or Stage 3 measurements were changed or added. The repair is documented in `dev-entries/phase-20.md`; a 2T/4T native-Windows lifecycle smoke remains required before Stage 3 may resume.
