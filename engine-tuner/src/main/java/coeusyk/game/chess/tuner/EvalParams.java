...
public final class EvalParams {
    ...
    public static double[] extractFromCurrentEval() {
        ...
        p[0]  = 100;  p[1]  = 85;    // Pawn (MG pinned at 100)
        p[2]  = 262; p[3]  = 217;   // Knight
        p[4]  = 276; p[5]  = 257;   // Bishop
        p[6]  = 362; p[7]  = 476;   // Rook
        p[8]  = 912; p[9]  = 756;  // Queen
        p[10] = 0;    p[11] = 0;     // King
        ...
        int[] PASSED_MG = {0, 16, 1, 1, 1, 1, 2, 0};
        int[] PASSED_EG = {0, 1, 9, 28, 48, 103, 96, 0};
        for (int i = 0; i < 6; i++) {
            p[IDX_PASSED_MG_START + i] = PASSED_MG[i + 1];
            p[IDX_PASSED_EG_START + i] = PASSED_EG[i + 1];
        }
        p[IDX_ISOLATED_MG] = 11;
        p[IDX_ISOLATED_EG] = 4;
        p[IDX_DOUBLED_MG]  = -2;
        p[IDX_DOUBLED_EG]  = 23;

        // --- King safety ---
        p[IDX_SHIELD_RANK2]   = 13;
        p[IDX_SHIELD_RANK3]   = 5;
        p[IDX_OPEN_FILE]      = 50;
        p[IDX_HALF_OPEN_FILE] = 9;
        p[IDX_ATK_KNIGHT]     = 5;
        p[IDX_ATK_BISHOP]     = 5;
        p[IDX_ATK_ROOK]       = 3;
        p[IDX_ATK_QUEEN]      = -1;
        ...
        p[IDX_BISHOP_PAIR_MG] = 60;   // Bishop pair MG
        p[IDX_BISHOP_PAIR_EG] = 26;   // Bishop pair EG
        p[IDX_ROOK_7TH_MG]          = 1;    // Rook on 7th rank MG
        p[IDX_ROOK_7TH_EG]          = 5;   // Rook on 7th rank EG
        p[IDX_ROOK_OPEN_FILE_MG]    = 50;   // Rook on open file MG
        p[IDX_ROOK_OPEN_FILE_EG]    = -2;    // Rook on open file EG
        p[IDX_ROOK_SEMI_OPEN_MG]    = 25;   // Rook on semi-open file MG
        p[IDX_ROOK_SEMI_OPEN_EG]    = 13;   // Rook on semi-open file EG
        p[IDX_KNIGHT_OUTPOST_MG]    = 40;   // Knight outpost MG
        p[IDX_KNIGHT_OUTPOST_EG]    = 30;   // Knight outpost EG
        p[IDX_CONNECTED_PAWN_MG]    = 5;    // Connected pawn bonus MG
        p[IDX_CONNECTED_PAWN_EG]    = 4;    // Connected pawn bonus EG
        p[IDX_BACKWARD_PAWN_MG]     = 1;    // Backward pawn penalty MG
        p[IDX_BACKWARD_PAWN_EG]     = -1;    // Backward pawn penalty EG
        p[IDX_ROOK_BEHIND_PASSER_MG] = 7;  // Rook behind passer MG
        p[IDX_ROOK_BEHIND_PASSER_EG] = 7;   // Rook behind passer EG
